...
class CounteRGANExplainer(Explainer):
    ...
    def explain(self, instance, oracle: Oracle, dataset: Dataset):
        ...
        with torch.no_grad():
            ...
            self.generator.eval()
            torch_cf = self.generator(torch_data_instance)
    ...
    def save_explainer(self):
        torch.save(self.generator.state_dict(), os.path.join(self.explainer_store_path, self.name, 'explainer'))

    def load_explainer(self):
        self.generator.load_state_dict(torch.load(os.path.join(self.explainer_store_path, self.name, 'explainer')))
...
class ResidualGenerator(nn.Module):
    ...
    def init_run(self, kernels, strides, paddings):
        ...
        for i in range(len(kernels)):
            retr = (retr - 2*paddings[i] - kernels[i]) // strides[i] + 1
